...
## 0b. Desktop renderer UI-QA (browser harness) — recommended on this box

The Pear desktop GUI can't open a window on this machine (see PROGRESS.md — the Electron bundle-loader `SyntaxError`), so visual QA of the renderer happens **in a plain browser against a simulated main process**. This exercises the real `src/main.js`, `renderer.js`, `map2d.js`, `globe-renderer.js`, contacts panel, NEW badges, staleness colors, and offline-queue UI.

```bash
cd ichnaea-v2
npm run qa          # builds browser-qa/qa-bundle.js + harness.html, then serves on :8765
# open http://localhost:8765/browser-qa/harness.html
```

- The stub pipe (`browser-qa/stub-pipe.js`) answers `boot` with fixture state (4 contacts across active/stale/offline/never + a self pin + 2 queued check-ins) and replays live pushes (peer status, a fresh Alex check-in → NEW badge, a self check-in, an offline-queue sync). No P2P, no GPS.
- **Checklist:** contacts render with real 4-word fingerprints; staleness dots are green (Alex) / red (Blair, Casey) / none (Drew, "never"); a NEW badge appears on Alex; the status line reads "1 contact connected"; the GPS line reads "Location: checked in just now"; the queue line shows "Synced 2 offline check-ins"; switching the map style in Settings toggles the 3D globe (WebGL) vs the 2D map.
- **To verify the 3D globe specifically:** Settings → Map style → a `globe-*` style. A WebGL-capable browser (Chrome/Firefox) renders it; a software-rendering fallback message shows if WebGL is unavailable.
- The harness is read-only (no writes to `data/`); inspect simulated pipe traffic in devtools console. Build artifacts are gitignored (`qa-bundle.js`, `harness.html`) and regenerated by `npm run qa`.
