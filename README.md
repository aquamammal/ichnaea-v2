...
- **"Ask them to check in" (opt-in location request):** each contact row has an **Ask** button that asks that contact to broadcast a check-in. It only reaches them over an **active, verified connection** (offline → the UI says so). They only honor it if they've enabled **Settings → "Honor location requests from contacts"** (default **OFF**); when off, your ask is **silently ignored** (no reply at all). Asks are **rate-limited to 5 min per contact** both ways, and a honored ask triggers the contact's normal scheduled-style check-in (precision snapping, at-rest encryption, and the offline queue all apply). Deliberately not a "force" — honoring is always the receiver's choice.
